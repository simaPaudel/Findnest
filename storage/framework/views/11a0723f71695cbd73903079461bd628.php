<?php $__env->startSection('title', 'Roommate Matches'); ?>
<?php $__env->startSection('page-title', 'Roommate Matches'); ?>
...
<div class="space-y-6">
    <div class="fn-card p-6 md:p-8">
        <div class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            ...
            <a href="<?php echo e(route('user.roommate-preferences.edit')); ?>" class="fn-btn-secondary shrink-0">
                Edit Preferences
            </a>
        </div>
        ...
        <div class="mt-6 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
            <div class="rounded-xl border border-slate-200 p-4">
                ...
                <p class="mt-1 text-sm font-semibold text-slate-900"><?php echo e($preference->budget_range ?: 'Not set'); ?></p>
            </div>
            <div class="rounded-xl border border-slate-200 p-4">
                <p class="text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">Location</p>
                <p class="mt-1 text-sm font-semibold text-slate-900"><?php echo e($preference->preferred_location ?: 'Not set'); ?></p>
            </div>
            <div class="rounded-xl border border-slate-200 p-4">
                <p class="text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">Gender Preference</p>
                <p class="mt-1 text-sm font-semibold capitalize text-slate-900"><?php echo e($preference->gender_preference ?: 'Any'); ?></p>
            </div>
            <div class="rounded-xl border border-slate-200 p-4">
                <p class="text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">Matches</p>
                <p class="mt-1 text-sm font-semibold text-slate-900"><?php echo e($matches->count()); ?> result<?php echo e($matches->count() === 1 ? '' : 's'); ?></p>
            </div>
        </div>
    </div>
    ...
    <?php if($matches->isEmpty()): ?>
        <div class="fn-card p-8 text-center">
            ...
            <div class="mt-6">
                <a href="<?php echo e(route('user.roommate-preferences.edit')); ?>" class="fn-btn-primary">
                    Edit Preferences
                </a>
            </div>
        </div>
                    ...
                    <div class="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
                        <div class="flex min-w-0 items-start gap-4">
                            <div class="flex h-14 w-14 shrink-0 items-center justify-center overflow-hidden rounded-2xl bg-slate-100 font-semibold text-slate-500">
                                <?php if($photoUrl): ?>
                                    <img src="<?php echo e($photoUrl); ?>" alt="<?php echo e($match['name']); ?>" class="h-full w-full object-cover">
                                <?php else: ?>
                                    <span><?php echo e(strtoupper(substr($match['name'] ?? 'U', 0, 1))); ?></span>
                                <?php endif; ?>
                            </div>

                            <div class="min-w-0">
                                <div class="flex flex-wrap items-center gap-2">
                                    <h3 class="text-lg font-semibold text-slate-950"><?php echo e($match['name']); ?></h3>
                                    <span class="fn-badge <?php echo e($scoreClass); ?>"><?php echo e($score); ?>% Match</span>
                                </div>

                                <p class="mt-1 text-sm text-slate-500">
                                    <?php echo e($match['preferred_location'] ?: 'Location not specified'); ?>

                                    <?php if(!empty($match['budget_range'])): ?>
                                        <span class="mx-2 text-slate-300">&middot;</span>
                                        <span><?php echo e($match['budget_range']); ?></span>
                                    <?php endif; ?>
                                </p>

                                <?php if(!empty($match['reasons'])): ?>
                                    <div class="mt-4 flex flex-wrap gap-2">
                                        <?php $__currentLoopData = $match['reasons']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $reason): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        ...
                        <div class="mt-5 flex flex-wrap gap-3">
                            <button
                                ...
                                data-roommate-user-id="<?php echo e($match['user_id']); ?>"
                            >
                                View Profile
                            </button>
                            ...
                            <button
                                ...
                                data-roommate-user-id="<?php echo e($match['user_id']); ?>"
                                data-contact-url="<?php echo e(route('user.conversations.roommate.create-or-open', ['userId' => $match['user_id']])); ?>"
                            >
                                Contact
                            </button>
                        </div>
...
<script>
    document.addEventListener('DOMContentLoaded', () => {
        const roommateProfiles = <?php echo json_encode($roommateProfiles, 15, 512) ?>;
        ...
        const handleContactOpen = (button) => {
            ...
            contactForm.dataset.redirectUrl = <?php echo json_encode(route('user.messages.index'), 15, 512) ?>;
...
<?php echo $__env->make('user.layout', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\FindNest\resources\views/roommates/matches.blade.php ENDPATH**/ ?>