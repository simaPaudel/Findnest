...
class UpdatePropertyRequest extends FormRequest
{
    ...
    protected function withValidator($validator)
    {
        $validator->after(function ($validator) {
            ...
            // If rental_mode is 'per_room', at least one room must exist
            if ($this->rental_mode === 'per_room') {
                $existingRooms = $property->rooms()->count();
                $newRoomsCount = count($this->rooms ?? []);

                if ($existingRooms === 0 && $newRoomsCount === 0) {
                    $validator->errors()->add(
                        'rooms',
                        'At least one room must exist for room-based rental modes'
                    );
                }
            }
            ...
            // Warn if changing rental_mode would invalidate existing rooms
            if ($property->rental_mode !== $this->rental_mode) {
                if ($this->rental_mode === 'full_property' && $property->rooms()->count() > 0) {
                    $validator->errors()->add(
                        'rental_mode',
                        'This property has rooms. Changing to "full_property" mode will make those rooms unavailable for booking.'
                    );
                }
            }
        });
